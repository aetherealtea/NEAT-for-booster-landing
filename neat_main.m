...
%list of parameters
  ...
  %parameters of main file
    maxgeneration=2000; %maximum number of generations for generational loop    
  ...
  %reproduction parameters
    ...
    %mutation
    mutation.probability_add_node=0.03; %0.03 initial
    mutation.probability_add_connection=0.05; %0.05 initial
...
while generation<maxgeneration & flag_solution==0
   ...
   % call evaluation function , fitnesses of individuals will be stored in population(:).fitness
   ...
   time_start=toc; 
   population=model_simulation(population);
   %population=fulladder_experiment(population);
    time_finish=toc;
    time_cycle=[time_cycle,time_finish-time_start];
   ...
   if maximale_fitness>9999

      flag_solution=1;
   end
   index=find(([population(:).fitness]==top_fitness))
   if (best_individual.fitness<population(index).fitness)
    best_individual.nodegenes=population(index).nodegenes;
    best_individual.connectiongenes=population(index).connectiongenes;
    best_individual.fitness=population(index).fitness;
    best_individual.species=population(index).species;
   end;

   best_fit=[best_fit, best_individual.fitness];
   ...
   ylabel('max fitness');
   subplot(2,3,4);
   plot(max_overall_fitness(2,:),time_cycle(:));
   ylabel('time for sim');
   subplot(2,3,5);
      plot(max_overall_fitness(2,:),best_fit(:));
   ylabel('best fit');
   drawnow;   
